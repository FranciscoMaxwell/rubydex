...
impl Drop for NameTracker<'_> {
    fn drop(&mut self) {
        for name_id in self.tracked_name_ids.drain(..) {
...
pub struct TrackedName<'graph> {
    tracker: NameTracker<'graph>,
    name_id: NameId,
}

impl<'graph> TrackedName<'graph> {
    fn new(tracker: NameTracker<'graph>, name_id: NameId) -> Self {
        TrackedName { tracker, name_id }
    }
...
/// Takes a constant name and nesting names (e.g.: `["Foo", "Bar::Baz", "Qux"]`) and transforms them into a `NameId`,
/// registering each required part in the graph. The names are untracked when the returned guard is dropped.
/// Returns `None` if the constant name contains no valid identifier parts (e.g.: `""`, `"::"`, `"Foo::"`).
pub fn name_from_nesting_names<'graph>(
    graph: &'graph mut Graph,
    const_name: &str,
    nesting: Vec<String>,
) -> Option<TrackedName<'graph>> {
    ...
    Some(TrackedName::new(tracker, name_id))
}

/// Takes a constant name and an existing lexical nesting and transforms them into a `NameId`, registering each required
/// part in the graph. The names are untracked when the returned guard is dropped.
pub fn name_from_lexical_nesting<'graph>(
    graph: &'graph mut Graph,
    const_name: &str,
...
mod tests {
    ...
    fn nesting_is_converted_to_name_id() {
        ...
        let tracked_name = name_from_nesting_names(
            &mut graph,
            "Some::CONST",
        ...
        let qux_name = tracked_name.graph().names().get(&some_name.nesting().unwrap()).unwrap();
        assert_eq!(StringId::from("Qux"), *qux_name.str());
        assert!(qux_name.parent_scope().is_none());

        let zip_name = tracked_name.graph().names().get(&qux_name.nesting().unwrap()).unwrap();
        assert_eq!(StringId::from("Zip"), *zip_name.str());

        let bar_name = tracked_name
            .graph()
            .names()
        ...
        let foo_name = tracked_name.graph().names().get(&bar_name.nesting().unwrap()).unwrap();
    ...
    fn top_level_reference_is_converted_to_name_id() {
        ...
        let foo_name = tracked_name
            .graph()
            .names()
            .get(&const_name.nesting().unwrap())
            .unwrap();
        assert_eq!(StringId::from("Foo"), *foo_name.str());
        assert!(foo_name.nesting().is_none());
        assert!(foo_name.parent_scope().is_none());
    ...
    fn singleton_class_names_use_attached_parent_scope() {
        ...
        let singleton_name = tracked_name
            .graph()
            .names()
            .get(&const_name.nesting().unwrap())
            .unwrap();
    ...
    fn invalid_names_restore_name_and_string_counts() {
        ...
        assert!(name_from_nesting_names(&mut graph, "", vec!["Foo".into()]).is_none());
        assert_eq!(name_count, graph.names().len());
        assert_eq!(string_count, graph.strings().len());

        assert!(name_from_nesting_names(&mut graph, "Foo::", vec!["Bar".into()]).is_none());
        assert_eq!(name_count, graph.names().len());
        assert_eq!(string_count, graph.strings().len());

        assert!(name_from_lexical_nesting(&mut graph, "Foo::", None).is_none());
        assert_eq!(name_count, graph.names().len());
        assert_eq!(string_count, graph.strings().len());
    }
    ...
    #[test]
    fn tracked_name_is_untracked_when_scope_ends() {
        let mut graph = Graph::new();
        let name_count = graph.names().len();
        let string_count = graph.strings().len();

        {
            let tracked_name = name_from_nesting_names(&mut graph, "Bar::CONST", vec!["Foo".into()]).unwrap();
            let name_id = tracked_name.name_id();
            let const_name = tracked_name.graph().names().get(&name_id).unwrap();

            assert_eq!(
                "CONST",
                tracked_name.graph().strings().get(const_name.str()).unwrap().as_str()
            );
            assert_eq!(name_count + 3, tracked_name.graph().names().len());
            assert_eq!(string_count + 3, tracked_name.graph().strings().len());
        }
    ...
    #[test]
    fn tracked_name_is_untracked_during_unwind() {
        let mut graph = Graph::new();
        let name_count = graph.names().len();
        let string_count = graph.strings().len();
